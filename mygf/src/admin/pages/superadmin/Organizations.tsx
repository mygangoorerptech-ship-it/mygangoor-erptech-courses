              ...
              <tr key={o.id} className="border-t">
                ...
                <td className="p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    {o.status !== 'active' && (
                      <button
                        disabled={actionLoading === `active-${o.id}`}
                        className="px-2 py-1 rounded border disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 inline-flex items-center gap-1"
                        onClick={async () => {
                          if (actionLoading) return;

                          try {
                            setActionLoading(`active-${o.id}`);
                    ...
                    {o.status === 'active' && (
                      <button
                        disabled={actionLoading === `inactive-${o.id}`}
                        className="px-2 py-1 rounded border disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 inline-flex items-center gap-1"
                        onClick={async () => {
                          if (actionLoading) return;

                          try {
                            setActionLoading(`inactive-${o.id}`);
                    ...
                    {o.status !== 'suspended' && (
                      <button
                        disabled={actionLoading === `suspended-${o.id}`}
                        className="px-2 py-1 rounded border disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 inline-flex items-center gap-1"
                        onClick={async () => {
                          if (actionLoading) return;

                          try {
                            setActionLoading(`suspended-${o.id}`);
                    ...
                    <button
                      disabled={actionLoading === `edit-${o.id}`}
                      className="px-2 py-1 rounded border disabled:opacity-50 disabled:cursor-not-allowed hover:bg-slate-50 inline-flex items-center gap-1"
                      onClick={() => setOpen({ mode: 'edit', initial: o })}
                    >
                      <Pencil size={16} /> Edit
                    </button>
                    <button
                      ...
                      onClick={async () => {
                        if (
                          !confirm(
                            'Delete organization? This will unassign its students.'
                          )
                        ) {
                          return;
                        }
                        ...
                        try {
                          setActionLoading(`delete-${o.id}`);

                          await deleteOne(o.id);
...
function OrgModal({ open, mode, initial, onClose, onSubmit }: {
  ...
  return (
    <Modal open={open} onClose={onClose} title={mode === 'create' ? 'New organization' : 'Edit organization'}>
      <form className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[70vh] overflow-y-auto pr-1" onSubmit={async (e) => {
        e.preventDefault();
          ...
          <Button
            ...
            onClick={async () => {
              try {
                ...
                toast.success(
                  `${rows.length} organizations imported successfully`
                );
              } catch (err: any) {
                console.error(err);

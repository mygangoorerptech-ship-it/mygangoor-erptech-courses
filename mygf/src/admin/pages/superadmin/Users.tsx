    ...
    <div className="space-y-4">
                ...
                <tr key={u.id} className="border-t">
                  ...
                  <td className="p-3">
                    <div className="flex flex-wrap items-center gap-2">
                      {u.status !== 'active' && (
                        <button
                          className="px-2 py-1 rounded border hover:bg-slate-50 inline-flex items-center gap-1 disabled:opacity-60"
                          onClick={async () => {
                            if (actionLoading?.[`active-${u.id}`]) return;
                      ...
                      {u.status === 'active' && (
                        <button
                          className="px-2 py-1 rounded border hover:bg-slate-50 inline-flex items-center gap-1 disabled:opacity-60"
                          onClick={async () => {
                            if (actionLoading?.[`disable-${u.id}`]) return;
                      ...
                      {u.role === 'admin' && (
                        <button
                          className="px-2 py-1 rounded border hover:bg-slate-50 inline-flex items-center gap-1 disabled:opacity-60"
                          onClick={async () => {
                            if (actionLoading?.[`promote-${u.id}`]) return;
                      ...
                      {u.role === 'superadmin' && !isMe && (
                        <button
                          className="px-2 py-1 rounded border hover:bg-slate-50 inline-flex items-center gap-1 disabled:opacity-60"
                          onClick={async () => {
                            if (actionLoading?.[`demote-${u.id}`]) return;
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if (actionLoading?.[`delete-${u.id}`]) return;
      ...
      <CSVModal
        ...
        onImport={async (rows) => {
          ...
          toast.success(
            `${rows.length} users imported successfully`
          );
...
function UserDetailsModal({
  ...
  const profile = data?.formProfile || data?.data?.formProfile

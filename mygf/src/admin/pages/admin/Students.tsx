...
export default function ADStudents(){
  const qc = useQueryClient()
  const { user } = useAuth() as any
  const orgId: string | undefined = user?.orgId // if present, we filter to admin’s org
  ...
  const query = useQuery({
    queryKey:['students', { ...filters, orgId }],
    queryFn: ()=> listStudents({ q: filters.q || undefined, status: filters.status, orgId } as StudentFilters)
  })

  const createMut = useMutation({ mutationFn: createStudent, onSuccess: ()=> qc.invalidateQueries({ queryKey:['students'] }) })
  const updateMut = useMutation({ mutationFn: ({id, patch}:{id:string, patch: Partial<Student>})=> updateStudent(id, patch), onSuccess: ()=> qc.invalidateQueries({ queryKey:['students'] }) })
  const deleteMut = useMutation({ mutationFn: deleteStudent, onSuccess: ()=> qc.invalidateQueries({ queryKey:['students'] }) })
  const statusMut = useMutation({ mutationFn: ({id, status}:{id:string, status:StudentStatus})=> setStudentStatus(id, status), onSuccess: ()=> qc.invalidateQueries({ queryKey:['students'] }) })
  const bulkMut   = useMutation({ mutationFn: bulkUpsertStudents, onSuccess: ()=> qc.invalidateQueries({ queryKey:['students'] }) })
    ...
    <div className="space-y-4">
      <header className="grid gap-3 md:grid-cols-5">
        <div className="md:col-span-2 space-y-2">
          <Label>Search</Label>
          <Input placeholder="Name, username, email..." value={filters.q} onChange={(e)=> setFilters(f=> ({...f, q: e.target.value}))} />
        </div>
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50 text-slate-600">
            <tr>
              ...
              <th className="text-left font-medium p-3 w-[300px]">Actions</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(s => (
            ...
            {rows.length===0 && <tr><td className="p-6 text-center text-slate-500" colSpan={5}>No students</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      <CSVModal
        ...
        onImport={(rows)=> {
          // force orgId for admin if present
          const mapped = rows.map(r => ({ ...r, orgId: orgId || r.orgId }))
          bulkMut.mutate(mapped, { onSuccess: ()=> setCsvOpen(false) })
        }}
      />
    </div>
  )
...
function StudentModal({ open, mode, initial, orgId, onClose, onSubmit }:{
  open:boolean
  mode:'create'|'edit'
  initial?: Student
  orgId?: string
    ...
    <Modal open={open} onClose={onClose} title={mode==='create' ? 'Add student' : 'Edit student'}>
      <form className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[70vh] overflow-y-auto pr-1" onSubmit={(e)=>{
        ...
        onSubmit({
          ...
          status,
          orgId, // admin sticks to own org (if provided)
        } as any)
      }}>
        <div>
  ...
  const parseCSV = (raw: string) => {
    ...
    for (let i=1;i<lines.length;i++){
      ...
      const map: Record<string,string> = {
        ...
        orgid: 'orgId', orgname: 'orgName', status: 'status'
      }
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" onClick={()=> {
            const sample = `username,email,name,status
john,john@example.com,John Smith,active
mei,mei@beta.example,Mei Lin,active`
            setText(sample)
          }}>Load sample</Button>
        </div>
        <div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={()=> {
            ...
            if (rows.length===0) { alert('No valid rows parsed.'); return }
            onImport(rows)
          }}>Import</Button>
        </div>
      </div>

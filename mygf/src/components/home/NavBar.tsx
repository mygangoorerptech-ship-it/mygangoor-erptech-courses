    ...
    <header className="sticky top-0 z-50 bg-cadetBlue/90 backdrop-blur border-b border-blue-300/40">
      <nav className="flex items-center justify-between px-4 sm:px-8 py-4">
        ...
        <div className="hidden md:flex items-center gap-6 relative">
          {navItems.map(({ label, href, iconId, onClick }) => (
          ...
          {!isAuthenticated ? (
            ...
            <Link
              to="#profile"
              className="text-sm font-medium px-4 py-2 rounded-md border border-blue-300/40 hover:bg-white/10 bg-gradient-to-r from-green-300 via-teal-400 to-cyan-500 bg-clip-text text-transparent"
            >
              My Profile
            </Link>
          )}
        ...
        <div className="md:hidden bg-gradient-to-br from-white via-blue-50 to-blue-100 text-slate-900 px-4 py-3 space-y-2">
          ...
          {!isAuthenticated ? (
            ...
            <Link
              to="#profile"
              onClick={() => setMobileMenuOpen(false)}
              className="block px-4 py-2 rounded-md hover:bg-blue-200/50"
            >
              My Profile
            </Link>
          )}
